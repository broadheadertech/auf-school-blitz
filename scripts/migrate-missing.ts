...
async function migrate() {
  ...
  if (e1) console.log('  departments rpc failed, trying direct...', e1.message)

  // 2. Academic settings table
  ...
  if (e2) console.log('  academic_settings rpc failed...', e2.message)

  // 3. Enrollments table
  const { error: e3 } = await db.rpc('exec_sql', {
    sql: `
  ...
  if (e3) console.log('  enrollments rpc failed...', e3.message)

  console.log('  Note: rpc exec_sql may not exist. Creating via SQL editor fallback...\n')
  console.log('If rpc failed, run this SQL in Supabase SQL Editor:\n')
  printFallbackSQL()
}
